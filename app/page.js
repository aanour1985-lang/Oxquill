...
var LANGUAGES = ["English","Arabic","Spanish","French","German","Italian","Portuguese","Russian","Chinese","Japanese","Korean","Hindi","Turkish","Dutch","Swedish","Polish","Czech","Greek","Hebrew","Thai","Vietnamese","Indonesian","Malay","Filipino","Swahili","Persian","Urdu","Bengali"];
...
export default function Home() {
  ...
  function buildPrompt() {
    ...
    if (activeTool === "debate") {
      return "Create a detailed debate about: " + topic + ". Language: " + battleLang +
  ...
  async function handleGenerate() {
    var toolData = TOOLS.find(function (t) { return t.id === activeTool; });
    var cost = toolData ? toolData.cost : 5;

    if (credits < cost && !(user && user.isAdmin)) {
      showToast(t("nomoney"), "error");
      return;
    }
    if ((activeTool === "generate" || activeTool === "debate") && !topic.trim()) {
      showToast(t("enterTopic"), "error");
      return;
    }
    if (PASTE_TOOLS.indexOf(activeTool) !== -1 && !pasteText.trim() && activeTool !== "titles") {
    ...
    try {
      ...
      if (data.content) {
        ...
        if (!(user && user.isAdmin)) { setCredits(function (prev) { return prev - cost; }); }
        showToast(t("done") + " -" + cost + " credits", "success");

        /* #2 Add notification */
        var toolLabel = toolData ? (rtl ? toolData.ar : toolData.en) : activeTool;
        ...
        /* Save article if generate */
        if (activeTool === "generate" && safeDb && user) {
          try {
            await safeDb.saveArticle(user.id, topic, language, mood, era, wordCount, data.content);
          } catch (e) {}
        }
      } else {
              ...
              >
                ...
                <div>
                  ...
                  <div style={{ fontSize: "0.75rem", color: TH.primary }}>
                    {currentLevel.ic + " " + (rtl ? currentLevel.nA : currentLevel.n) + " \u2022 " + credits + " \ud83e\ude99"}
                  </div>
                </div>
              </div>
            ) : (
      ...
      <main style={{ paddingTop: "56px", minHeight: "calc(100vh - 140px)" }}>
              ...
              <div style={Object.assign({}, cardStyle, { marginTop: "24px", position: "relative" })}>
                ...
                {hasWatermark && (
                  <div style={{
                    ...
                    color: "rgba(91,108,240,0.06)", pointerEvents: "none",
                ...
                <div style={{
                  ...
                  <div style={{ display: "flex", gap: "8px" }}>
                    ...
                    <button
                      onClick={function () {
                        var content = result;
                        if (hasWatermark) content = content + "\n\n---\n" + t("watermarkText");
                    ...
                    {canShare && (
                      <button
                        ...
                        style={Object.assign({}, btnBase, {
                          ...
                          color: "#22C55E", padding: "6px 16px", borderRadius: "8px", fontSize: "0.85rem",
                        })}
                      >{"\ud83d\udce4 " + t("share")}</button>
                    )}
                  </div>
        ...
        {view === "roast" && (
          <div style={{ padding: "24px 16px 40px", maxWidth: "660px", margin: "0 auto", textAlign: "center" }}>
            ...
            {/* Roast button */}
            <button
              onClick={function () { setActiveTool("roast"); handleGenerate(); }}
              disabled={generating}
              onMouseEnter={onHoverIn} onMouseLeave={onHoverOut} onMouseDown={onPress}
              style={Object.assign({}, btnBase, {
                width: "100%", padding: "16px", borderRadius: "14px", border: "none",
        ...
        {view === "credits" && (
          <div style={{ padding: "24px 16px 40px", maxWidth: "560px", margin: "0 auto" }}>
            <div style={{
              ...
              <div style={{ fontSize: "2.8rem", fontWeight: 800 }}>{credits + " \ud83e\ude99"}</div>
              <button
                onClick={function () { navigate("pricing"); }}
                onMouseEnter={onHoverIn} onMouseLeave={onHoverOut}
                style={Object.assign({}, btnBase, {
