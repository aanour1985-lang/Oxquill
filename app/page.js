    ...
    <div style={{maxWidth:900,margin:"0 auto",padding:"20px 16px 40px"}}>
      ...
      {p.profileTab==="settings" && (
        <ProfileSettings profile={p.profile} rtl={p.rtl} saveProfileSettings={p.saveProfileSettings} handleDeleteAccount={p.handleDeleteAccount}/>
            ...
            {p.profileTab==="levels" && (
              <Card>
                <h3 style={{fontSize:15,fontWeight:700,marginBottom:16}}>{p.rtl?"\ud83c\udfc6 \u062e\u0631\u064a\u0637\u0629 \u0627\u0644\u0645\u0633\u062a\u0648\u064a\u0627\u062a":"\ud83c\udfc6 Level Roadmap"}</h3>
                {LEVELS.map(function(lv,i){
                  var reached = p.xp >= lv.xp;
                  var isCurrent = p.level.lv === lv.lv;
                  return (
                    <div key={i} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 8px",borderRadius:10,marginBottom:4,background:isCurrent?"var(--pS)":reached?"var(--cd)":"transparent",border:isCurrent?"2px solid var(--pr)":"1px solid "+(reached?"var(--bd)":"transparent"),opacity:reached?1:.5}}>
                      <div style={{fontSize:18,minWidth:28,textAlign:"center"}}>{["\ud83c\udf31","\u270f\ufe0f","\ud83d\udcdd","\u2b50","\ud83c\udfc6","\ud83d\udc51","\ud83c\udf1f","\ud83d\udc8e","\ud83e\udde0","\ud83e\uddb8"][i]||"\u2b50"}</div>
                      <div style={{flex:1}}>
                        <div style={{fontWeight:700,fontSize:12}}>{"Lv"+lv.lv+" \u2014 "+lv.n}</div>
                        <div style={{fontSize:10,color:"var(--t3)"}}>{lv.xp+" XP \u2022 "+lv.goal+" \u2022 "+lv.reward}</div>
                      </div>
                      {reached && <span style={{color:"var(--ok)",fontWeight:700,fontSize:14}}>\u2713</span>}
                    </div>
                  );
                })}
              </Card>
